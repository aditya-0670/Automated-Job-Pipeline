...
async def test_state_persists_to_postgres(saver):
    ...
    config = thread_config("cp-persist")

    await graph.ainvoke(state_for("cp-persist", job_url="https://kept.example"), config=config)
...
async def test_a_new_graph_object_resumes_the_same_session(saver):
    ...
    """
    config = thread_config("cp-recover")

    first = build_graph(checkpointer=saver)
    await first.ainvoke(state_for("cp-recover", job_url="https://survives.example"), config=config)
...
async def test_resume_continues_rather_than_restarting(saver):
    ...
    config = thread_config("cp-resume")
    graph = build_graph({SCRAPER_KEYWORD: counting_node}, checkpointer=saver)

    await graph.ainvoke(state_for("cp-resume"), config=config)
    assert runs == ["scraper"]
...
async def test_llm_work_is_not_repeated_after_a_crash(saver):
    ...
    config = thread_config("cp-nowaste")
    ...
    await graph.ainvoke(state_for("cp-nowaste"), config=config)
...
async def test_sessions_are_isolated_by_thread_id(saver):
    ...
    await graph.ainvoke(
        state_for("cp-user-a", job_url="https://a.example"), config=thread_config("cp-user-a")
    )
    await graph.ainvoke(
        state_for("cp-user-b", job_url="https://b.example"), config=thread_config("cp-user-b")
    )

    a = await graph.aget_state(thread_config("cp-user-a"))
    b = await graph.aget_state(thread_config("cp-user-b"))
...
async def test_checkpoint_history_is_retained(saver):
    ...
    config = thread_config("cp-history")
    graph = build_graph(checkpointer=saver)
    await graph.ainvoke(state_for("cp-history"), config=config)
...
async def test_events_accumulate_rather_than_overwrite(saver):
    ...
    config = thread_config("cp-events")
    graph = build_graph(
        {SCRAPER_KEYWORD: emit("first"), DATA_RETRIEVER: emit("second")},
        checkpointer=saver,
        interrupt_before=(HUMAN_REVIEW,),
    )
    await graph.ainvoke(state_for("cp-events"), config=config)
...
async def test_latest_state_reads_without_advancing(saver):
    ...
    await graph.ainvoke(state_for("cp-read"), config=thread_config("cp-read"))

    view = await latest_state(graph, "cp-read")
...
async def test_full_state_survives_serialisation(saver):
    ...
    config = thread_config("cp-roundtrip")
    graph = build_graph(checkpointer=saver)
    original = state_for(
        "cp-roundtrip",
