...
| `llm.py` | ✅ | `LLMProvider` ABC + `GeminiProvider` (tenacity retry with exponential backoff) + `MockProvider`. The mock is not a constant stub — it inspects the prompt and returns evaluator-shaped JSON or echoes back the LaTeX block, so downstream nodes receive plausibly shaped input and stay genuinely testable. `complete_json()` tolerates markdown fences and brace-hunts as a last resort. `get_llm()` degrades to the mock when no key is set. |
